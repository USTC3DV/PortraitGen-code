...
class SMPLX(SMPLH):
    ...
    def __init__(
        ...
        **kwargs
        ...
        lmk_faces_idx = np.loadtxt(os.path.join(model_path, 'pts51_faces_idx.txt'), dtype=np.int64)
        lmk_bary_coords = np.loadtxt(os.path.join(model_path, 'pts51_bary_coords.txt'), dtype=np.float32)
    ...
    def forward(
        ...
        pose2rot: bool = True,
        add_rot = None,
        with_pose_blendshape = False, 
        with_rott_return = False,
        **kwargs
    ) -> SMPLXOutput:
        '''
        ...
        if transl is None:
            if hasattr(self, 'transl'):
                transl = self.transl.expand(batch_size, -1)

        if self.use_pca:
            print(left_hand_pose.shape, self.left_hand_components.shape)
            left_hand_pose = torch.einsum(
                'bi,ij->bj', [left_hand_pose, self.left_hand_components])
            right_hand_pose = torch.einsum(
                'bi,ij->bj', [right_hand_pose, self.right_hand_components])

        # body_pose = torch.cat((torch.zeros_like(body_pose[:, :5*3]), body_pose[:, 5*3:]), dim=1)
        ...
        if not with_rott_return:
            vertices, joints = lbs(shape_components, full_pose, self.v_template,
                                shapedirs, self.posedirs,
                                self.J_regressor, self.parents,
                                self.lbs_weights, pose2rot=pose2rot, with_pose_blendshape=with_pose_blendshape
                                )
        else:
            vertices, joints, verts_rott, v_shaped = lbs(shape_components, full_pose, self.v_template,
                                shapedirs, self.posedirs,
                                self.J_regressor, self.parents,
                                self.lbs_weights, pose2rot=pose2rot, with_pose_blendshape=with_pose_blendshape, with_rott_return = True
                                )
        ...
        if not with_rott_return: 
            ...
            return output, verts_rott, v_shaped
