...
from . import coco_wholebody, smplx
...
def convert_kps(
    ...
    src: str = 'smplx',
    dst: str = 'coco_wholebody',
    approximate: bool = False,
    confidence: Union[np.ndarray, torch.Tensor] = None,
    ...
    if confidence is not None:
        assert type(confidence) == type(keypoints)
    else:
        confidence = new_array_func(
            shape=(keypoints.shape[0], keypoints.shape[1]),
            value=1,
    ...
    out_confidence = new_array_func(
        shape=(confidence.shape[0], len(dst_names)),
        value=0,
    ...
    out_confidence[:, dst_idxs] = confidence[:, src_idxs]
    out_shape = extra_dims + (len(dst_names),)
    out_confidence = out_confidence.reshape(out_shape)

    return out_keypoints, out_confidence
